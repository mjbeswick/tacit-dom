...
describe('errorBoundary', () => {
  it('should wrap a component and return it normally when no errors occur', () => {
    const TestComponent = createReactiveComponent(() => div('Test content'));
  ...
  it('should catch synchronous errors and render fallback UI', () => {
    const BuggyComponent = createReactiveComponent(() => {
      throw new Error('Test error');
    });
    ...
    expect(element.querySelector('p')?.textContent).toBe('Test error');
  ...
  it('should use custom fallback UI when provided', () => {
    ...
    const customFallback = (error: Error) =>
      div(
        ...
        span('Custom error message: ' + error.message),
      );
    ...
    expect(element.className).toBe('custom-error');
    expect(element.textContent).toBe('Custom error message: Custom error');
  });
  ...
  it('should log to console by default when error occurs', () => {
    const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
  ...
  it('should handle non-Error objects thrown', () => {
    ...
    expect(element.querySelector('p')?.textContent).toBe('String error');
  });

  it('should allow manual error triggering', () => {
    const TestComponent = createReactiveComponent(() => div('Normal content'));
    const SafeComponent = errorBoundary(TestComponent);
    ...
    expect(element.querySelector('p')?.textContent).toBe('Manual error');
  });
  ...
  it('should allow error state reset', () => {
    ...
    // Should show error again since the component still throws
    expect(element.querySelector('h2')).toBeTruthy();
  });

  it('should preserve component props when rendering fallback', () => {
    const BuggyComponent = createReactiveComponent<{ name: string }>(
      (props) => {
        if (props?.name === 'trigger') {
    ...
    // Normal render
    let element = SafeComponent({ name: 'Alice' });
    expect(element.textContent).toBe('Hello Alice');

    // Error render
    element = SafeComponent({ name: 'trigger' });
    expect(element.querySelector('h2')).toBeTruthy();

    // Try again should work with same props
    const tryAgainButton = element.querySelector('button');
    if (tryAgainButton) {
      tryAgainButton.click();
      // The component should re-render with the same props
      // and still throw an error, so we should see the fallback again
    }
  });

  it('should mark elements with error boundary data attribute', () => {
    const TestComponent = createReactiveComponent(() => div('Test'));
    const SafeComponent = errorBoundary(TestComponent);

    const element = SafeComponent();
    expect(element.getAttribute('data-error-boundary')).toBe('true');
  });

  it('should handle _setContainer properly', () => {
    const TestComponent = createReactiveComponent(() => div('Test'));
    const SafeComponent = errorBoundary(TestComponent);

    const container = document.createElement('div');
    SafeComponent._setContainer(container);

    expect(container.getAttribute('data-error-boundary')).toBe('true');
  });
  ...
  it('should have proper toString representation', () => {
    ...
    expect(SafeComponent.toString()).toContain('[ErrorBoundary:');
  });
});
