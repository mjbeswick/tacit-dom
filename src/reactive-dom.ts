...
// Change detection for reactive updates
const reactiveValues = new WeakMap<HTMLElement, Map<string, any>>();

// Global update counter to prevent infinite rerenders
let globalUpdateCount = 0;
const MAX_GLOBAL_UPDATES = 10000; // Maximum updates per cycle
...
function hasValueChanged(
  ...
  // Quick check for reference equality first
...
const reactiveNodes = new WeakMap<
  HTMLElement,
  Array<{ signal: Signal<any> | Computed<any>; unsubscribe: () => void }>
>();

/**
 * WeakMap to track reactive list nodes for cleanup.
 *
 * Maps HTML elements to arrays of signal/computation subscriptions
 * for reactive lists, including the container element.
 */
