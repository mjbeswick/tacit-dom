(function (window) {
  ...
  function setCurrentPattern() {
    ...
    while (nextPat >= player.xm.patterns.length) {
      if (player.cur_songpos + 1 < player.xm.songpats.length) {
        ...
        (player.cur_songpos === player.xm.song_looppos && player.cur_songpos !== 0) ||
  ...
  function nextRow() {
    ...
    if (player.cur_pat == -1 || player.cur_row >= player.xm.patterns[player.cur_pat].length) {
      ...
      player.next_row = 1;
      player.cur_songpos++;
      if (player.cur_songpos >= player.xm.songpats.length) player.cur_songpos = player.xm.song_looppos;
      setCurrentPattern();
    }
  ...
  function nextTick() {
    ...
    for (j = 0; j < player.xm.nchan; j++) {
      ...
      if (isNaN(ch.period)) {
        console.log(
          prettify_notedata(player.xm.patterns[player.cur_pat][player.cur_row][j]),
          'set channel',
          j,
          'period to NaN',
        );
  ...
  function audio_cb(e) {
    ...
    var offset = 0;
    var ticklen = 0 | ((f_smp * 2.5) / (player.xm.bpm * (player.speed || 1.0)));
    ...
    while (buflen > 0) {
      ...
      for (j = 0; j < player.xm.nchan; j++) {
        ...
        VU[j] = MixChannelIntoBuf(player.xm.channelinfo[j], offset, offset + tickduration, dataL, dataR) / tickduration;
