...
function WelcomeComponent() {
  ...
  return div(
    h1(t('welcome')),
    p(t('hello', 'Hello, {name}!', { name: 'World' })),
    nav(
      a({ href: '#' }, t('navigation.home')),
      a({ href: '#' }, t('navigation.about')),
      a({ href: '#' }, t('navigation.contact')),
    ),
    div(
      button({ onclick: () => setLocale('es') }, 'Switch to Spanish'),
      button({ onclick: () => setLocale('fr') }, 'Switch to French'),
    ),
  );
}
```
...
function WelcomeComponent() {
  ...
  return div(
    h1(t('welcome.title', 'Welcome!')),
    p(t('welcome.subtitle', 'Choose your language')),
    div(
...
function ContactForm() {
  ...
  return form(
    div(
      ...
      span({ className: 'error' }, nameError),
    ),
    div(
      ...
      span({ className: 'error' }, emailError),
    ),
    button({ type: 'submit' }, t('form.submit', 'Submit')),
  );
}
```
...
function ItemList() {
  ...
  return div(
    ...
    ul(...items.get().map((item, index) => li({ key: index }, item))),
  );
}
```
...
function Dashboard() {
  ...
  return div(
    h1(t('dashboard.title', 'Dashboard')),
    div(
      p(
